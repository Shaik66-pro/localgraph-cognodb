...
const GET_GRAPH_STATS = `
  CALL { MATCH (b:Business) RETURN count(b) AS totalBusinesses }
  CALL { MATCH (u:User) RETURN count(u) AS totalUsers }
  CALL { MATCH (r:Review) RETURN count(r) AS totalReviews }
  CALL { MATCH (c:City) RETURN count(c) AS totalCities }
  CALL { MATCH (cat:Category) RETURN count(cat) AS totalCategories }
  RETURN totalBusinesses, totalUsers, totalReviews, totalCities, totalCategories
`;

// 2. Search & filter businesses (parameterized)
...
const GET_BUSINESS_BY_ID = `
  MATCH (b:Business {business_id: $business_id})-[:LOCATED_IN]->(c:City)
  OPTIONAL MATCH (b)-[:HAS_CATEGORY]->(cat:Category)
...
const GET_BUSINESS_REVIEWS = `
  MATCH (u:User)-[:WROTE]->(r:Review)-[:ABOUT]->(b:Business {business_id: $business_id})
...
// 5. MAIN FEATURE: Similar Business Recommendations (2-Hop Traversal)
// Graph Traversal: Business A <-[:ABOUT]- Review <-[:WROTE]- User -[:WROTE]-> Review -[:ABOUT]-> Business B
...
// 6. MULTI-HOP GRAPH INSIGHT (3+ Hop Traversal)
// Traverses: Business A -> Category <- Business B <- Review <- User -> Review -> Business C
...
const GET_CATEGORIES = `
  MATCH (c:Category)
  RETURN c.name AS name, c.category_id AS category_id
  ORDER BY c.name ASC
  LIMIT 200
`;
...
const GET_CITIES = `
  MATCH (c:City)
  RETURN c.name AS name, c.state AS state
  ORDER BY c.name ASC
`;
...
const GET_FEATURED_BUSINESSES = `
  MATCH (b:Business)-[:LOCATED_IN]->(c:City)
  OPTIONAL MATCH (b)-[:HAS_CATEGORY]->(cat:Category)
  WHERE b.stars >= 4.5 AND b.review_count >= 10
  WITH b, c, collect(DISTINCT cat.name) AS categories
  RETURN b.business_id AS business_id,
         b.name AS name,
         b.address AS address,
